...
func {{.PoolConstructor}}(parent golang.Container) *{{.PoolName}} {
	createClient := func() ({{NameOf .Service.UserType}}, error) {
		graph := {{.ClientConstructor}}(parent.Context(), parent.CancelFunc(), nil, parent)
		var client {{NameOf .Service.UserType}}
		err := graph.Get("{{.InstanceName}}", &client)
		return client, err
	}
...
func (pool *{{$receiver}}) {{Signature $f}} {
	client, _ := pool.clients.Pop()
	// TODO: handle error in clients.Pop()
