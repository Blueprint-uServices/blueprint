...
func (pool *ClientPool) GenerateFuncs(module golang.ModuleBuilder) error {
	...
	graph, err := gogen.NewGraphBuilder(module, args.ClientFileName, args.PackageName, args.ClientConstructor)
	if err != nil {
		return err
	}
	...
	poolFileName := filepath.Join(module.Info().Path, args.PackageName, args.PoolFileName)
	return gogen.ExecuteTemplateToFile("goprocMain", poolTemplate, args, poolFileName)
}
...
func (pool *ClientPool) AddInstantiation(builder golang.GraphBuilder) error {
	...
	builder.Import(builder.Module().Info().Name + "/" + args.PackageName)
	...
	code, err := gogen.ExecuteTemplate("declareClientPool", buildPoolTemplate, args)
	if err != nil {
		return err
	}
...
func (pool *ClientPool) getTemplateArgs(module golang.ModuleBuilder) (*templateArgs, error) {
	...
	args.PackageName = "pool"
	args.ClientFileName = args.Service.Name + "_client.go"
	args.PoolFileName = args.Service.Name + "_pool.go"
	args.ClientConstructor = fmt.Sprintf("New_%v_Client", args.Service.Name)
	args.PoolConstructor = fmt.Sprintf("New_%v_Pool", args.Service.Name)
	args.Imports = gogen.NewImports(module.Info().Name + "/" + args.PackageName)
...
var buildPoolTemplate = `func(ctr golang.Container) (any, error) {
	return pool.{{.PoolConstructor}}(ctr), nil
}`
...
func {{.PoolConstructor}}(parent golang.Container) *{{.PoolName}} {
	createClient := func() ({{NameOf .Service.UserType}}, error) {
		...
		err = graph.Get("{{.WrappedClient}}", &client)
		return client, err
	}
