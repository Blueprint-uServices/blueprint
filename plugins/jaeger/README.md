...
1. The package provides a jaeger container that provides the server\-side implementation of the Jaeger collector.
2. Generates a go\-client for connecting to the jaeger server. The client is set as the default exporter for the opentelemetry tracer which is then used by the opentelemetry plugin to correctly export spans to the Jaeger collector.
...
<a name="Collector"></a>
## func [Collector](<https://github.com/blueprint-uservices/blueprint/blob/main/plugins/jaeger/wiring.go#L32>)

```go
func Collector(spec wiring.WiringSpec, collectorName string) string
```

Adds a jaeger docker container named \`collectorName\` that uses the latest jaeger:all\-in\-one container and generates the clients needed by the generated application to communicate with the server.
