...
The package provides a go\-client for generating traces on both the server and client side along with the ability to export traces to a custom collector such as jaeger or zipkin. The generated clients handle context propagation correctly on both the server and client sides.
