...
func newOpenTelemetryClientWrapper(name string, server golang.Service, collector OpenTelemetryCollectorInterface, genClientSpans bool) (*OpenTelemetryClientWrapper, error) {
	node := &OpenTelemetryClientWrapper{}
	node.WrapperName = name
	node.Wrapped = server
	node.Collector = collector
	node.outputPackage = "ot"
	node.GenClientSpans = genClientSpans
	return node, nil
}
...
func (node *OpenTelemetryClientWrapper) GenerateFuncs(builder golang.ModuleBuilder) error {
	...
	return generateClientHandler(builder, wrapped_iface, impl_iface, coll_iface, node.outputPackage, node.GenClientSpans)
}
...
func generateClientHandler(builder golang.ModuleBuilder, wrapped *gocode.ServiceInterface, impl *gocode.ServiceInterface, coll_iface *gocode.ServiceInterface, outputPackage string, genClientSpans bool) error {
	...
	server := &clientArgs{
		...
		GenSpans:        genClientSpans,
	}

	server.Imports.AddPackages("context", "go.opentelemetry.io/otel/trace")
