...
func (node *Process) GenerateArtifacts(workspaceDir string) error {
	...
	var all_nodes []ir.IRNode
	all_nodes = append(all_nodes, node.PriorityNodes...)
	...
	// Instantiate all priority nodes
	for _, node := range node.PriorityNodes {
		namespaceBuilder.PriorityInstantiate(node.Name())
	}
