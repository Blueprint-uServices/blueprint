...
// ThriftMethodName returns the method name as generated by the Thrift compiler
// Thrift appends underscore to methods starting with "New" to avoid keyword conflicts
func (c *clientArgs) ThriftMethodName(methodName string) string {
	capitalized := strings.Title(methodName)
	if strings.HasPrefix(capitalized, "New") {
...
func (client *{{$receiver}}) {{SignatureWithRetVars $f}} {
	...
	marshall_{{$service}}_{{$f.Name}}_req(req, {{ArgVars $f}})

	ctx, cancel := context.WithTimeout(ctx, client.Timeout)
	defer cancel()

	rsp, err := client.Client.{{$.ThriftMethodName $f.Name}}(ctx, req)
	if err != nil {
		err = ctx.Err()
	}
	if err != nil {
	...
	{{RetVarsEquals $f}} unmarshall_{{$service}}_{{$f.Name}}_rsp(rsp)
	return
}
{{end}}
`